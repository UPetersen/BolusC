...
@property (nonatomic, strong) UIBarButtonItem *theIcon;
@end
...
- (id)initWithFields:(NSArray *)fields
{
    if (self = [super initWithFrame:CGRectMake(0.0f, 0.0f, 320.0f, 44.0f)])
    {
        ...
        [self.toolbar setBarStyle:UIBarStyleDefault]; //         UIBarStyleBlackTranslucent];
...
//        [self setTheIcon:[[UIBarButtonItem alloc] initWithImage:ui style:<#(UIBarButtonItemStyle)#> target:<#(id)#> action:<#(SEL)#>]]
    //    UIImage *myImage = [UIImage imageNamed: @"Icon-72"];
        
        // TODO hier wieder intWithBarButtonSystemItem UIBarButtonItemStyleDone nehmen statt ...Plain und das Bild weglasse, dann muesste es wieder stimmen
        // TODO und die Bilder löschen, da sie nur errors erzeugen und nicht gebraucht werden.
     //   [self setDoneButton:[[UIBarButtonItem alloc] initWithImage:myImage style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:) ]];
     //   self.doneButton.image = myImage;
        
        // Uwi: so müsste das mit der Internationalisierung besser funktionieren
